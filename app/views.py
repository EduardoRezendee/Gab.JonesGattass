...
def home(request):
    ...
    revisoes_hoje = ProcessoAndamento.objects.filter(
        fase_fase_in=["Revisão", "Revisão Des"],
    ...
    if not general_metrics:
        ...
        processos_por_fase = (
            ProcessoAndamento.objects.filter(
                ...
                status_status_in=["Não iniciado", "Em andamento"]
            )
            .values('fase__fase')
    ...
    if is_revisor:
        ...
        processos_em_revisao = Processo.objects.filter(
            andamentos_fase_fase="Revisão",
            andamentos__usuario=user,
            concluido=False
        ).distinct().select_related('especie', 'usuario').prefetch_related('andamentos', 'andamentos_fase', 'andamentosstatus', 'andamentos_usuario')
        if numero_processo:
    ...
    # VISÃO DA DESEMBARGADORA
        ...
        processos_em_revisao_des = Processo.objects.filter(
            andamentos_fase_fase="Revisão Des",
            andamentos__usuario=user,
            concluido=False
        ).distinct().select_related('especie', 'usuario', 'tipo').prefetch_related(
            'andamentos', 'andamentos_fase', 'andamentosstatus', 'andamentos_usuario'
        )
        if numero_processo:
    ...
    # VISÃO DO ASSESSOR/USUÁRIO COMUM
        ...
        processos_nao_concluidos = Processo.objects.filter(
            usuario=user,
            concluido=False
        ).select_related('especie', 'usuario').prefetch_related('andamentos', 'andamentos_fase', 'andamentos_status')
        ...
        if tipo:
            processos_nao_concluidos = processos_nao_concluidos.filter(tipo_tipo_icontains=tipo)
        if especie:  # Novo filtro por espécie
            processos_nao_concluidos = processos_nao_concluidos.filter(especie_sigla_iexact=especie)
        ...
        processos_revisao_des = Processo.objects.filter(
            usuario=user,
            andamentos_fase_fase="Revisão Des",
            andamentos_dt_criacao_date=hoje
        ).distinct().select_related('especie', 'usuario').prefetch_related('andamentos', 'andamentos_fase', 'andamentos_status')
    ...
    tarefas_do_dia = TarefaDoDia.objects.filter(usuario=user).select_related('processo', 'processo_especie', 'processotema').prefetch_related('processoandamentos', 'processoandamentos_fase')
...
def get_revisoes_hoje_data(request):
    ...
    ).values('processo_usuariofirst_name', 'processousuario_last_name').annotate(
    ...
    labels = [f"{assessor['processo_usuariofirst_name']} {assessor['processousuario_last_name']}" for assessor in enviados_por_assessor]
...
def get_es_assessor_hoje_data(request):
    ...
    entradas_hoje = (
        Processo.objects.filter(data_dist_date=hoje, usuario_isnull=False)
        .values('usuario_first_name', 'usuario_last_name')
        .annotate(quantidade=Count('id'))
    )
    saidas_hoje = (
        Processo.objects.filter(dt_conclusao_date=hoje, concluido=True, usuario_isnull=False)
        .values('usuario_first_name', 'usuario_last_name')
        .annotate(quantidade=Count('id'))
    )
    ...
    assessores_nomes = list(set(
        [f"{e['usuario_first_name']} {e['usuario_last_name']}".strip() for e in entradas_hoje] +
        [f"{s['usuario_first_name']} {s['usuario_last_name']}".strip() for s in saidas_hoje]
    ))

    entradas_dict = {f"{e['usuario_first_name']} {e['usuario_last_name']}".strip(): e['quantidade'] for e in entradas_hoje}
    saidas_dict = {f"{s['usuario_first_name']} {s['usuario_last_name']}".strip(): s['quantidade'] for s in saidas_hoje}
...
def get_especies_data(request):
    ...
    especies_nomes = [e['especie_especie'] if e['especie_especie'] else "Sem Espécie" for e in processos_por_especie]
